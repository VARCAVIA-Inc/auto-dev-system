...
def manage_pull_requests():
    """
    Logica potenziata per la gestione completa delle Pull Request.
    Cerca le PR, controlla lo stato dei test e agisce di conseguenza (merge o chiusura).
    """
    ...
    try:
        ...
        for pr in prs:
            ...
            # Lo stato può essere PENDING, SUCCESS, o FAILURE
            ...
            if status == 'SUCCESS':
                ...
                logging.warning(f"PR #{pr_number} ha fallito i test. Chiudo la PR.")
...
def main():
    # (Anche la funzione main rimane invariata, la ometto ma va inclusa nel file)
    setup_logging()
