...
def get_gemini_model(model_name: str) -> GenerativeModel:
    ...
    try:
        if not _is_vertex_ai_initialized:
            if not PROJECT_ID: raise ValueError("Variabile d'ambiente GCP_PROJECT non impostata.")
...
def generate_response(model: GenerativeModel, prompt: str) -> str:
    try:
        ...
        if not response.candidates: raise ValueError("La risposta dell'API non contiene candidati validi.")
