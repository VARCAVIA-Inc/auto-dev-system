...
def main():
    ...
    # Inizializzazione
    GITHUB_TOKEN = os.getenv("GITHUB_TOKEN"); OPENAI_API_KEY = os.getenv("OPENAI_API_KEY"); GITHUB_REPOSITORY = os.getenv("GITHUB_REPOSITORY")
    ...
    for phase_index, phase in enumerate(business_plan.get('phases', [])):
        for task_index, task in enumerate(phase.get('tasks', [])):
            ...
            if task_status == 'pending' and task.get('agent') == 'ProjectBot':
                print(f"Trovato task PENDING: {task_description}"); run_project_bot(task, task_index, phase_index); return
                ...
                try:
                    with open(plan_path, 'r+') as f:
                        ...
                        # --- CONTROLLO DI VALIDITÀ DEL PIANO ---
                        has_pending_subtasks = any(line.strip().startswith('- [ ]') for line in lines)
                        if not has_pending_subtasks:
                            # Se non ci sono più task da delegare, controlla se sono tutti fatti o se il piano è invalido
                            is_plan_finished = all('[✅]' in line for line in lines if line.strip().startswith('- ['))
                            if is_plan_finished:
                                ...
                                print("Piano di sviluppo non valido o bloccato (nessun task da fare ma non tutti completati). Rigenero.")
                                os.remove(plan_path)
                                update_main_task_status(task_index, phase_index, "pending")
                                push_changes_to_main(repo_root, f"fix: Piano non valido per '{task_description[:30]}...'. Reimposto per rigenerazione.")
                            return
                        # --- FINE CONTROLLO ---

                        # (Logica di Supervisione PR invariata) ...
                        plan_changed = False
                                ...
                                if pr_title.startswith("feat: Implementa '"):
                                    ...
                                    for i, line in enumerate(lines):
                                        if subtask_key in line and '[▶️]' in line:
                                            lines[i] = line.replace('[▶️] In Progress', '[✅] Fatto!'); plan_changed = True
                                            print(f"Sotto-task '{subtask_key}' completato (PR unita)."); break
                        if plan_changed:
                            f.seek(0); f.truncate(); f.writelines(lines)
                            push_changes_to_main(repo_root, f"chore: Aggiorna piano, task completati")

                        # (Logica di Delega invariata) ...
                        for i, line in enumerate(lines):
                            if line.strip().startswith('- [ ]'):
                                subtask_description = line.replace('- [ ]', '').strip()
                                print(f"Delego il sotto-task: '{subtask_description}'")
                                branch_name = f"operator/task-{int(time.time())}"; commit_message = f"feat: Implementa '{subtask_description[:30]}...'"
                                env = os.environ.copy(); env['GH_TOKEN'] = GITHUB_TOKEN
                                ...
                                lines[i] = lines[i].replace('[ ]', '[▶️] In Progress')
                                f.seek(0); f.truncate(); f.writelines(lines)
                                push_changes_to_main(repo_root, f"chore: Avviato sotto-task '{subtask_description[:30]}...'")
                                return

                except FileNotFoundError:
                    print(f"Errore: development_plan.md non trovato. Reimposto a 'pending'.")
                    update_main_task_status(task_index, phase_index, "pending")
